...
.DESCRIPTION
    ...
    - Prompts with a Save File dialog to choose where the results CSV should be written
      (folder + file name). The save dialog opens in the input CSV folder when possible.
    ...
    - Includes each user's Email Address (mail) and Employee ID in the export.
...
function Select-OutputCsv {
    ...
    $initialDir = $null
    if (-not [string]::IsNullOrWhiteSpace($SuggestedDirectory) -and (Test-Path -LiteralPath $SuggestedDirectory)) {
        $initialDir = (Resolve-Path -LiteralPath $SuggestedDirectory).Path
    }
    if ([string]::IsNullOrWhiteSpace($initialDir)) {
        $initialDir = Get-DefaultPickerDirectory
    }
...
function Get-UserEmailAddress {
    ...
    foreach ($name in @('EmailAddress', 'mail')) {
        $value = Get-AdPropertyText -User $User -Name $name
...
function New-LastLogonReportRow {
    param(
        [Parameter(Mandatory)]
...
function Invoke-LastLogonSelfTest {
    ...
    $schema = New-LastLogonReportRow -SamAccountName 'jdoe'
    Assert-Equal ($schema.Contains('EmailAddress')) $true 'schema has EmailAddress'
    Assert-Equal ($schema.Contains('EmployeeID')) $true 'schema has EmployeeID'
    Assert-Equal $schema['EmailAddress'] $null 'EmailAddress starts null'
    Assert-Equal $schema['EmployeeID'] $null 'EmployeeID starts null'
    ...
    Assert-Equal (Get-UserEmailAddress -User $fakeUser) 'jane.doe@contoso.com' 'prefer EmailAddress trimmed'
    Assert-Equal (Get-AdPropertyText -User $fakeUser -Name 'EmployeeID') 'E12345' 'EmployeeID trimmed'

    $mailOnly = [PSCustomObject]@{ mail = 'only-mail@contoso.com' }
    Assert-Equal (Get-UserEmailAddress -User $mailOnly) 'only-mail@contoso.com' 'fall back to mail'
...
if ([string]::IsNullOrWhiteSpace($OutputCsv)) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($InputCsv)) {
        try {
            $suggestDir = Split-Path -Parent -LiteralPath $InputCsv
        }
        catch {
            $suggestDir = $null
        }
    }
    $OutputCsv = Select-OutputCsv -SuggestedDirectory $suggestDir
}
...
foreach ($sam in $samAccountNames) {
    ...
    $row = New-LastLogonReportRow -SamAccountName $sam

    if ($adUsers.ContainsKey($sam)) {
        $user = $adUsers[$sam]
        $row.DisplayName       = Get-AdPropertyText -User $user -Name 'DisplayName'
        $row.EmailAddress      = Get-UserEmailAddress -User $user
        $row.EmployeeID        = Get-AdPropertyText -User $user -Name 'EmployeeID'
        $row.UserPrincipalName = Get-AdPropertyText -User $user -Name 'UserPrincipalName'
        $row.DistinguishedName = Get-AdPropertyText -User $user -Name 'DistinguishedName'
