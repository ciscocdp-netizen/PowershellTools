...
function New-HtmlReport {
    ...
    function Build-Table {
        param($Rows, [string[]]$Columns, [string]$EmptyMessage)
        ...
        $head = ($Columns | ForEach-Object { "<th>$(ConvertTo-HtmlEncoded $_)</th>" }) -join ''
        ...
        foreach ($row in $rowList) {
            $cells = foreach ($col in $Columns) {
                ...
                if ($value -is [bool]) {
                    ...
                    "<td>$(ConvertTo-HtmlEncoded ([string]$value))</td>"
                }
            }
    ...
    $detailColumns = @(
        ...
        'UseDatabaseQuotaDefaults', 'ProhibitSendReceiveGB', 'ProhibitSendGB', 'IssueWarningGB',
...
<main>
  ...
  <section>
    ...
    <div class="scroll">$(Build-Table -Rows $notCompliant -Columns $detailColumns -EmptyMessage 'All licensed mailboxes meet the storage target.')</div>
  </section>

  <section>
    <h2>E3 mailboxes approaching the $($Summary.LegacyQuotaGB) GB limit (&ge; $($Summary.NearLimitPercent)%)</h2>
    <div class="scroll">$(Build-Table -Rows $nearLimit -Columns $nearColumns -EmptyMessage 'No E3 mailboxes are near the legacy cap.')</div>
  </section>
</main>
