...
.EXAMPLE
    .\Get-EntraUsersWithoutAuthMethod.ps1 -DeviceCode -TenantId contoso.onmicrosoft.com

.EXAMPLE
    # Same script saved as AuthMethodReport.ps1
    .\AuthMethodReport.ps1 -DeviceCode -TenantId contoso.onmicrosoft.com

.EXAMPLE
    # PowerShell ISE: open this script and press F5. File dialogs and browser sign-in are used automatically.

.NOTES
    This file is also published as AuthMethodReport.ps1 (same script). If you
    already saved a copy under that name, overwrite it with this version.
    ...
    Sign-in: a non-elevated Windows console can use Web Account Manager (WAM).
    PowerShell ISE, an Administrator window, and -DeviceCode never use WAM or
    Connect-MgGraph -UseDeviceAuthentication (those hang and never show a code).
    They request a device code over HTTPS and print the URL/code immediately.
...
function Test-IsSpecificEntraTenant {
    ...
    if ([string]::IsNullOrWhiteSpace($Value)) { return $false }
    $t = $Value.Trim()
    if ($t -match '^(?i:common|organizations|consumers)$') { return $false }
    return $true
}

function Resolve-DeviceCodeTenant {
    $candidate = $TenantId
    if (-not (Test-IsSpecificEntraTenant -Value $candidate)) {
        Write-Host "Device-code sign-in needs your Entra tenant ID or domain." -ForegroundColor Yellow
        Write-Host "Do not use 'organizations' or 'common' — Microsoft will not issue a code." -ForegroundColor Yellow
        $candidate = ''
        if ($script:WinFormsLoaded) {
            try {
                ...
                $candidate = [Microsoft.VisualBasic.Interaction]::InputBox(
                    "Enter your Entra tenant ID (GUID) or domain.`r`n`r`nExample: contoso.onmicrosoft.com`r`n`r`nDo not use organizations or common.",
                    'Entra tenant',
                    ''
                )
            }
        ...
        if ([string]::IsNullOrWhiteSpace($candidate)) {
            $candidate = Read-Host 'Enter Entra tenant ID or domain (contoso.onmicrosoft.com)'
        }
    }

    if (-not (Test-IsSpecificEntraTenant -Value $candidate)) {
        throw 'Device-code sign-in requires -TenantId with your Entra tenant GUID or domain (not organizations/common).'
    }

    $script:TenantId = $candidate.Trim()
...
function Get-OAuthErrorCode {
    ...
    $obj = $null
    try { $obj = $Result.Object } catch { }
    if ($null -eq $obj -and -not [string]::IsNullOrWhiteSpace([string]$Result.Text)) {
        try { $obj = $Result.Text | ConvertFrom-Json } catch { }
    }
...
function Invoke-EntraFormPost {
    ...
    $req.UserAgent = "AuthMethodReport/$($script:ScriptBuild)"
    ...
    try {
        ...
        if ($webEx.Response) {
            ...
            throw ("HTTPS request to Microsoft timed out or failed: {0}" -f $webEx.Message)
        }
    }
...
function Connect-ViaSystemBrowser {
    ...
    try {
        ...
        $tokError = Get-OAuthErrorCode -Result $tokResult
        if ($tokResult.StatusCode -lt 200 -or $tokResult.StatusCode -ge 300 -or -not $token -or -not [string]::IsNullOrWhiteSpace($tokError)) {
            $detail = $tokResult.Text
            if ([string]::IsNullOrWhiteSpace($detail)) { $detail = "HTTP $($tokResult.StatusCode)" }
            throw "Authorization-code token exchange failed: $detail"
        }
...
function Connect-ViaRestDeviceCode {
    ...
    # Both can hang forever in elevated Windows PowerShell / ISE and never print a code.
...
function Invoke-ConnectMgGraph {
    # WAM only. Never pass the SDK device-code switch — it hangs on elevated Windows.
    $cmd = Get-Command Connect-MgGraph -ErrorAction Stop
    $params = @{
...
function Connect-EntraGraph {
    ...
    if ($skipWam) {
        if (Test-IsPowerShellIse) {
            Write-Host "PowerShell ISE detected. Skipping WAM and requesting a Microsoft device code." -ForegroundColor Cyan
        }
        elseif (Test-IsProcessElevated) {
            Write-Host "Administrator session detected. Skipping WAM (it fails when elevated)." -ForegroundColor Cyan
        }
        elseif ($DeviceCode) {
            Write-Host "-DeviceCode specified. Skipping WAM." -ForegroundColor Cyan
        }
        Write-Host "If the next lines do not include 'Script build: $($script:ScriptBuild)', you are running an old copy of this file." -ForegroundColor Yellow
...
function Update-GraphAccessToken {
    ...
    try {
        ...
        $tokResult = Invoke-EntraFormPost -Uri "$authority/oauth2/v2.0/token" -Body $tokBody -TimeoutMs 20000
        $token = $tokResult.Object
        if ($tokResult.StatusCode -lt 200 -or $tokResult.StatusCode -ge 300 -or -not $token) {
            return $false
        }
        $accessToken = [string](Get-GraphResponseProperty -Response $token -Name 'access_token')
...
function Invoke-SelfTest {
    ...
    Assert-True (Test-IsSpecificEntraTenant 'e18c199f-7bc4-489e-b810-eddee385899b') 'tenant guid is specific'
    Assert-True (Test-IsSpecificEntraTenant 'contoso.onmicrosoft.com') 'tenant domain is specific'
    Assert-False (Test-IsSpecificEntraTenant 'organizations') 'organizations is not specific'
    Assert-False (Test-IsSpecificEntraTenant 'common') 'common is not specific'
    Assert-False (Test-IsSpecificEntraTenant '') 'blank tenant is not specific'

    $pendingJson = '{"error":"authorization_pending","error_description":"wait"}'
    ...
    Assert-True ($deviceFn -match 'Connect-ViaRestDeviceCode') 'device-code flow uses REST'

    $restFn = ${function:Connect-ViaRestDeviceCode}.ToString()
    ...
    Assert-True ($wamFn -notmatch 'UseDeviceAuthentication') 'WAM helper does not use SDK device-code'
    Assert-True ($wamFn -notmatch [regex]::Escape($hangPhrase)) 'WAM helper does not print the hanging device-code message'

    $scriptSource = Get-Content -LiteralPath $PSCommandPath -Raw
    Assert-True ($scriptSource -notmatch [regex]::Escape($hangPhrase)) 'script no longer prints the hanging device-code prompt'
    Assert-True ($scriptSource -match [regex]::Escape($script:ScriptBuild)) 'script build stamp is present'
