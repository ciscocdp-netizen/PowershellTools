...
Assert-True ($offboard -match 'Test-GraphAccountDisabled') 'Graph disable check treats bool and string accountEnabled values'
Assert-True ($offboard -match '\$delaysMs = @\(1000, 2000, 3000, 4000\)') 'Graph sign-in block retries verification after PATCH'
Assert-True ($offboard -notmatch 'verification shows account still enabled') 'Stale Graph read-back is not reported as a failed sign-in block'
