...
.USAGE
    powershell -ExecutionPolicy Bypass -STA -File .\AD-Delta-Compare.ps1
#>
...
# ---------------------------------------------------------------------------
# Shared value-normalization function text (injected into the runspace)
# ---------------------------------------------------------------------------
$script:NormFuncText = @'
function Convert-AdValueToString {
    ...
    if ($Value -is [System.Collections.IEnumerable] -and -not ($Value -is [string])) {
        $items = @()
        foreach ($v in $Value) { $items += (Convert-AdValueToString $v) }
...
function Write-Audit {
    ...
    $line = ('{0}  {1}  {2}' -f (Get-Date).ToString('yyyy-MM-dd HH:mm:ss'), $env:USERNAME, $Message)
...
$script:CompareScript = {
    param($Pdc, $R1, $R2, $Type, $Filter, $SearchBase, $Dn, $Zone, $DiffOnly, $NormText, $UserProps, $CompProps, $GroupProps, $NonRestorable)
            ...
            { $_ -in @('Users','Computers','Groups') } {
                ...
                foreach ($guid in $allGuids) {
                    ...
                    foreach ($attr in $props) {
                        $pv = if ($oP) { Convert-AdValueToString $oP.$attr } else { '<object missing>' }
                        $v1 = if ($o1) { Convert-AdValueToString $o1.$attr } else { '<object missing>' }
                        $v2 = if ($o2) { Convert-AdValueToString $o2.$attr } else { '<object missing>' }
            ...
            'GroupPolicy' {
                ...
                foreach ($id in $allIds) {
                    ...
                    foreach ($field in $fields) {
                        $pv = if ($gP) { Convert-AdValueToString $gP.$field } else { '<object missing>' }
                        $v1 = if ($g1) { Convert-AdValueToString $g1.$field } else { '<object missing>' }
                        $v2 = if ($g2) { Convert-AdValueToString $g2.$field } else { '<object missing>' }
                        $present = @()
                ...
                foreach ($key in $dcMap.Keys) {
                    ...
                    foreach ($m in $meta) {
                        $t[$m.AttributeName] = ("v{0}; {1}; from {2}" -f $m.Version, `
                            (Convert-AdValueToString $m.LastOriginatingChangeTime), $m.LastOriginatingChangeDirectoryServerIdentity)
                    }
                    $tables[$key] = $t
                }
...
function Start-Comparison {
    ...
    [void]$script:PowerShell.AddArgument($script:NormFuncText)
...
function Show-BulkRestoreDialog {
    ...
    # $Items: array of pscustomobjects with .Object .Attribute .PdcVal .R1Val .R2Val
    ...
    $rbR1.Add_CheckedChanged({ & $script:BulkPreviewRefresh })
    $rbR2.Add_CheckedChanged({ & $script:BulkPreviewRefresh })
    & $script:BulkPreviewRefresh
...
$btnRestore.Add_Click({
    ...
    if ($targets.Count -eq 1) {
        ...
        if ($res.Ok) {
            ...
            Write-Audit ("RESTORE obj='{0}' guid='{1}' attr='{2}' action='{3}' source='{4}' target-PDC='{5}' oldPDCval='{6}'" -f `
                $t.Object, $t.Guid, $t.Attribute, $res.Action, $sourceDc, $pdc, $t.PdcVal)
    ...
    foreach ($t in $targets) {
        ...
        if ($res.Ok) {
            ...
            Write-Audit ("BULK-RESTORE obj='{0}' guid='{1}' attr='{2}' action='{3}' source='{4}' target-PDC='{5}' oldPDCval='{6}'" -f `
                $t.Object, $t.Guid, $t.Attribute, $res.Action, $sourceDc, $pdc, $t.PdcVal)
            $t.Row.Cells['PDC'].Value = if ($sourceDc -eq $r1Name) { $t.R1Val } else { $t.R2Val }
        } else {
            $fail++
