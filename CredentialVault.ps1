...
function Get-VaultCredentialTemplates {
    return @(
        ...
        [pscustomobject]@{
            Id          = 'ExchangeCreds'
            Title       = 'Exchange Online service account'
            Summary     = 'Username, Password'
            DefaultFile = 'creds.enc'
            Fields      = @(
                [pscustomobject]@{ Name = 'Username'; Secret = $false; Prompt = 'Exchange Online UPN (e.g. svcIAM@contoso.com)' }
                [pscustomobject]@{ Name = 'Password'; Secret = $true;  Prompt = 'Password' }
            )
        }
        [pscustomobject]@{
...
function Test-CredentialVault {
    ...
    try {
        ...
        $templates = @(Get-VaultCredentialTemplates)
        if ($templates.Count -ne 5) {
            throw "Expected 5 credential templates, got $($templates.Count)."
        }
        $exchange = $templates | Where-Object { $_.Id -eq 'ExchangeCreds' } | Select-Object -First 1
        if (-not $exchange) {
            throw 'ExchangeCreds template is missing.'
        }
        $exchangeNames = @($exchange.Fields | ForEach-Object { $_.Name })
        foreach ($need in @('Username', 'Password')) {
            if ($exchangeNames -notcontains $need) {
                throw "ExchangeCreds template missing field $need"
            }
        }
